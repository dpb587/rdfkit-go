...
var defaultPrefixMap = rdfacontext.NewWidelyUsedInitialContext()
...
func parseTag(tag string, prefixes iri.PrefixMapper) (*tagInfo, error) {
	if tag == "" {
		return nil, nil
	}
...
func expandPredicate(predicate string, prefixes iri.PrefixMapper) rdf.IRI {
	...
	if colonIdx := strings.Index(predicate, ":"); colonIdx > 0 {
		...
		if expanded, ok := prefixes.ExpandPrefix(iri.PrefixReference{
			Prefix:    prefix,
			Reference: reference,
		}); ok {
