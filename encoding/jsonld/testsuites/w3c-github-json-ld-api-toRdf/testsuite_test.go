	...
	for _, sequence := range testdataManifest.Sequences {
		t.Run(string(sequence.ID), func(t *testing.T) {
			...
			decodeAction := func() (encodingtest.QuadStatementList, error) {
				...
				if strings.HasSuffix(string(sequence.ID), "#tnt02") {
					// input is not valid JSON; U+0080, as a control code, is supposed to be escaped
					dopt = dopt.SetParserOptions(inspectjson.TokenizerConfig{}.SetLax(true))
				}
