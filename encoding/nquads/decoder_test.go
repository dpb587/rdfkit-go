...
// 	s := NewDecoder(strings.NewReader(`<http://example/s><http://example/p><http://example/o>.
...
// _:s<http://example/p>_:bnode1.`), DecoderOptions{})
...
func TestReader_ScanIRIREF(t *testing.T) {
...
func TestReader_ScanString(t *testing.T) {
	for _, tc := range []struct {
		InputString   string
