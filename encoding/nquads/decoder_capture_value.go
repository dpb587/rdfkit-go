...
func (r *Decoder) captureSubjectOrGraphValue(grammarRule grammar.R) (rdf.SubjectValue, *cursorio.TextOffsetRange, error) {
	for {
		...
		if err != nil {
			return nil, nil, grammarRule.Err(r.newOffsetError(err, nil, nil))
		}
		...
		case r0 == '_':
			...
			if r1 != ':' {
				return nil, nil, grammarRule.Err(r.newOffsetError(cursorioutil.UnexpectedRuneError{Rune: r1}, []rune{r0}, []rune{r1}))
			}
		...
		case unicode.IsSpace(r0):
			r.commit([]rune{r0})
		default:
			return nil, nil, grammarRule.Err(r.newOffsetError(cursorioutil.UnexpectedRuneError{Rune: r0}, nil, []rune{r0}))
		}
	}
}
